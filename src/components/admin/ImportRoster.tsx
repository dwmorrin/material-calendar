/**
 * Possible replacement for the existing RosterRecords importer.
 * This one takes the same shape of data that the Users importer takes,
 * assumes that all the Users have already been added,
 * all the courses & sections have already been added,
 * and focuses on enrollments.
 */
...
const onSubmit: BulkImporter = (setSubmitting, dispatch, records) => {
  ...
  fetch(`${RosterRecord.url}/new-roster-import`, {
    method: "POST",
    ...
    .then(({ error, data }) => {
      ...
      dispatch({
        ...
        payload: {
          resources: {
            [ResourceKey.Courses]: (data as RosterRecord[]).map(
...
const ImportRoster: FunctionComponent<AdminUIProps> = ({ dispatch, state }) => {
  ...
  return (
    <Dialog fullScreen open={state.importRosterIsOpen}>
      <Toolbar>
        <IconButton
          edge="start"
          color="inherit"
          aria-label="close"
          onClick={(): void =>
            dispatch({ type: AdminAction.CloseImportRoster })
          }
        >
          <CloseIcon />
        </IconButton>
        <Typography variant="h5">Import Roster</Typography>
      </Toolbar>
      <Paper>
